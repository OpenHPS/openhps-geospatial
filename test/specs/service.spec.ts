...
import 'mocha';
import { Absolute2DPosition, GeographicalPosition, MemoryDataService } from '@openhps/core';
import { Building, Floor, Corridor, Room, SymbolicSpace, SymbolicSpaceService } from '../../src';
...
describe('SymbolicSpaceService', () => {
    ...
    const base = new Floor('0').setBuilding(building).setFloorNumber(0);
    ...
    it('should find the best space', (done) => {
        service
            .findSymbolicSpaces(lab.toPosition())
            .then((results) => {
    ...
    it('should find the best space when a geographical position is provided on a geojson source', (done) => {
        service
            ...
            .then(() => {
                return Promise.all(
                    GEOJSON.features
                        .map((feature) => SymbolicSpace.fromGeoJSON(feature))
